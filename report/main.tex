...
%  中文字型：使用 ctex 內建 Fandol 字型；Overleaf 編譯器請選 XeLaTeX
% =============================================================
\documentclass[12pt, a4paper, fontset=fandol]{ctexart}

% ---------- 中文字型（Overleaf / XeLaTeX）----------
% 若 Overleaf 中文無法正常顯示，請確認：Menu → Compiler = XeLaTeX。
% Fandol 為 TeX Live 內建中文字型，Overleaf 通常可直接使用。
...
\begin{enumerate}
  ...
  \item \textbf{資料路徑分工與效能隔離。} 如何協同 XDP 與 TC：由 XDP 負責早期封包解析、黑名單檢查與 SYN flood 相關處理，由 TC 在流統計較完整時執行分位桶推論與評分，避免將高成本邏輯放入最早期封包路徑。
...
\begin{enumerate}
  ...
  \item \textbf{設計 XDP／TC／userspace 分層管線。} 本專題整合 Python 離線訓練與模型匯出、Rust 使用者空間的模型載入器／記錄器／邊界更新器（BoundaryUpdater），以及 eBPF XDP／TC 資料路徑。XDP 負責早期封包解析、黑名單檢查與 SYN flood 相關處理；TC 負責 Contract5 特徵重建與評分表查表；使用者空間則負責模型參數下發、事件記錄與邊界校準，使早期封包處理、flow-level scoring、模型載入與策略更新各自位於合適層級。
...
\subsubsection{機器學習式入侵偵測與 DDoS 偵測}
大規模 NIDS 資料集與特徵工程研究提供了模型訓練與評估基礎。例如 BigFlow-NIDS 建構了包含 55{,}835{,}210 條流量紀錄的大型樣本庫~\cite{uddin2026bigflow}，而特徵選取研究亦整理了 NIDS 中常見的過濾式（filter）、包裹式（wrapper）與嵌入式（embedded）方法~\cite{verma2022feature}。在 DDoS 偵測方面，既有碩士論文研究已嘗試以監督式機器學習進行分散式阻斷服務攻擊偵測~\cite{sun2019ml}，以自編碼器重構誤差結合孤立森林偵測未知型 DDoS~\cite{zhao2024autoencoder}，並以深度神經網路搭配特徵優化提升偵測效果~\cite{wu2024dnn}。這些研究印證了異常偵測與孤立森林在 DDoS 場景的可行性，但其推論皆於使用者空間以完整模型執行，未處理核心資料路徑的部署限制。本專題承接此類工作對流層級（flow-level）特徵與孤立森林的重視，但將評估範圍收斂於 CIC-IDS-2019，並將重心移至核心快速路徑的可部署性。

\subsubsection{eBPF／XDP 資料路徑與其限制}
...
\subsubsection{特徵標準化、特徵縮減與跨環境限制}
...
既有 eBPF DDoS 防禦多強調 XDP 的早期丟棄能力；然而，所有邏輯皆放入 XDP 可能使最早期封包路徑承擔過多狀態與推論成本。本專題採 XDP／TC 分工：XDP 負責早期封包解析、黑名單檢查、SYN flood 相關處理與必要的輕量狀態更新；TC 則在流統計較完整時執行 Contract5 特徵重建與評分表查表。此分工使早期阻斷與模型評分各自位於較合適的資料路徑掛載點。
...
\begin{figure}[H]
  ...
  \caption{系統整體閉環控制架構。XDP／TC 負責核心快速路徑；Rust 使用者空間負責模型參數下發、日誌與邊界校準；完整孤立森林不在執行期資料路徑中執行。}
  \label{fig:arch}
\end{figure}

\subsubsection{資料流程說明}
封包進入系統後，首先由 XDP 快速路徑進行早期封包解析、黑名單比對、SYN-cookie 相關處理與必要的輕量狀態更新；若封包未被早期丟棄，後續由 TC 掛載點在流統計較完整時重建部署規格所需的五個特徵。決策層將特徵依照分位桶邊界轉換為 5-bit 索引，並查詢 32-entry 評分表取得異常分數。執行層根據分數與門檻執行 \code{PASS}、\code{MONITOR}、\code{RATE\_LIMIT} 或 \code{DROP}。回饋層則位於使用者空間，負責接收取樣事件、判斷正常漂移或攻擊污染，並在不中斷資料路徑的前提下更新分位桶邊界。
...
\begin{enumerate}
  \item 由 XDP／TC 更新每條流的連線狀態。
  \item TC 評分器依據連線狀態重建 Contract5 特徵。
  \item 每個特徵與對應分位桶邊界比較，得到一個分位桶位元（bucket bit）。
  \item 五個分位桶位元依固定位元順序組成 5-bit 索引。
  \item 以索引查詢 32-entry 評分表，取得異常分數。
  \item 分數與門檻比較後映射為 \code{PASS}、\code{MONITOR}、\code{RATE\_LIMIT} 或 \code{DROP}。
\end{enumerate}

此設計將完整樹狀模型的執行期推論替換為 $O(1)$ 查表。需特別說明的是，本報告使用 quantile-bucket distillation 描述離線異常偵測邏輯被壓縮為核心查表部署格式的工程流程；現況部署表由二值特徵模型產生，嚴格的 Contract5 continuous teacher 到 bucket student 之 knowledge distillation 仍屬後續工作。

\subsection{XDP／TC 資料路徑分工}
XDP 掛載點位於封包進入 Linux 網路堆疊之前，適合執行早期封包解析、黑名單檢查、SYN-cookie 相關處理與早期丟棄；但若將所有狀態累積與模型評分皆放入 XDP，會使最早期封包路徑承擔過多計算與驗證器風險。因此，本系統將流統計較完整時才需要的評分器放在 TC 掛載點中執行。

此分工可概括為：

\begin{itemize}
  \item \textbf{XDP：} 早期解析、黑名單、SYN-cookie、早期丟棄、連線狀態更新。
  \item \textbf{TC：} Contract5 特徵重建、5-bit 索引產生、評分表查表。
...
\subsection{核心資料路徑實作}
核心端資料路徑由 XDP 與 TC 共同構成。XDP 主程式負責早期封包解析、黑名單檢查、SYN-cookie 相關處理與早期丟棄；通過 XDP 的封包會更新每條流的連線狀態，使系統能累積後續 Contract5 所需的統計量。TC 掛載點則在流統計較完整時執行評分器，將連線狀態中的統計量轉換為 Contract5 特徵，並透過 5-bit 分位桶索引查詢 32-entry 評分表。此設計將高成本的樹狀模型推論移出資料路徑，使核心快速路徑僅保留整數比較、位元組合與 BPF map 查表。
...
\subsection{離線機器學習流程}
離線機器學習流程使用 CIC-IDS-2019 僅正常（BENIGN-only）訓練資料建立孤立森林相關模型，並產出核心可用的分位桶邊界與評分表。本研究區分三種 model class：Contract5 continuous teacher 作為 teacher / reference model；Abs20／Full25 作為不可部署的 baseline upper bound；deployed binarized student 則是 5-bit 全二值化 32-entry 查表模型。現況部署表由二值特徵模型產生；嚴格的 Contract5 continuous teacher 到 bucket student 之正式 knowledge distillation 仍屬後續工作。因此，本報告使用 quantile-bucket distillation 描述將離線異常偵測邏輯壓縮為核心查表部署格式的工程流程，但不將其誇大為完整 teacher-student distillation 已完成。
...
目前核心資料結構、XDP／TC 資料路徑、模型 JSON 載入、評分表查表、邊界更新器狀態機與可於主機測試的整合流程已實作。Linux 端曾完成基本端到端執行，包含 eBPF 程式載入、XDP／TC 掛載、日誌環形緩衝區（ring buffer）讀取與優雅關閉（graceful shutdown）。仍待補充的是完整的實機封包驗證、自適應邊界觸發測試、黑名單行為驗證，以及吞吐量、每秒封包數（pps）與 CPU 額外負擔等效能量測。因此，本報告在成果章節僅將目前系統定位為可部署的快速路徑雛形與範圍內實驗驗證，而不宣稱完整的生產級 DDoS 防護基準測試已完成。
...
\subsection{硬軟體規格}
本節保留實驗環境表格，待最終 Linux 驗證機器確定後填入 CPU、記憶體、網卡、核心版本與工具鏈版本。由於 eBPF 驗證器行為與核心版本、clang／LLVM 版本及網卡驅動有關，最終報告應以實際驗證環境為準。
\begin{table}[H]
  \centering
  \caption{實驗硬軟體環境規格}
  \label{tab:spec}
  ...
  \begin{tabularx}{\textwidth}{l X}
    ...
    記憶體           & （待填） \\
    網路卡           & （待填） \\
    作業系統 / 核心版本 & （待填，eBPF 需 Linux 5.x 以上） \\
    eBPF 工具鏈      & （待填，如 libbpf / clang 版本） \\
    使用者空間語言   & Rust \\
    離線訓練環境     & Python，scikit-learn、Polars、Pandas \\
    \bottomrule
  \end{tabularx}
\end{table}
...
此結果不等同於已完成核心端奈秒級延遲或整機吞吐量基準測試；其意義在於確認模型壓縮後的整數查表路徑相較完整 IF 推論具有明顯成本差距。完整的 XDP／TC 實機吞吐量、CPU 使用率、map lookup latency 與偵測至丟棄的端到端緩解延遲，仍列為後續驗證工作。

\subsection{實機效能量測規劃}
為補足目前 userspace proxy 僅能提供方向性成本比較的限制，本專題預留實機效能量測區域。待最終 Linux 驗證環境與流量產生器確定後，將以相同模型與相同 eBPF 程式量測核心端 fast-path 的吞吐量、CPU 額外負擔、map lookup latency 與端到端緩解延遲。表~\ref{tab:hardware-perf-plan} 目前作為待補實驗表格，數據填入前不作為結論依據。

\begin{table}[H]
  \centering
  \caption{實機效能量測項目與待補結果}
  ...
  \begin{tabularx}{\textwidth}{l l X}
    ...
    \midrule
    Packet throughput（pps） & 待測 & 比較 no mitigation、static blocklist、userspace IF 與 eBPF bucket policy 的最大穩定封包率 \\
    CPU usage（\%） & 待測 & 在相同流量負載下量測 eBPF enforcement 對 CPU 與 softirq 的額外負擔 \\
    Kernel map lookup latency（ns） & 待測 & 量測 SCORE\_TABLE 與 QUANTILE\_BOUNDS 查表路徑的核心端延遲分布 \\
    Map update latency（$\mu$s） & 待測 & 量測使用者空間更新分位桶邊界與版本切換的成本 \\
    Mitigation latency（$\mu$s） & 待測 & 量測偵測到開始 DROP／RATE\_LIMIT 的端到端延遲 \\
    Legitimate drop ratio（\%） & 待測 & 以 benign + attack 混流驗證實機環境下正常流量誤丟比例 \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\subsection{成果定位與限制}
本專題的主要成果是建立一條可部署於 eBPF 資料路徑的低延遲機器學習式 DDoS 快速路徑：Contract5 特徵可由核心連線狀態重建，五個分位桶位元可組成 32-entry 評分表，並由 Rust 使用者空間進行模型載入與邊界更新。系統在 CIC-DDoS2019 上能有效覆蓋多數 UDP 反射／放大攻擊，但不宣稱解決所有 DDoS 型態。Syn、UDP-lag、WebDDoS 與跨資料集 HOIC／BigFlow 泛化皆屬限制或範圍外觀察。此外，完整的實機吞吐量、核心奈秒級延遲與 CPU 額外負擔仍待最終驗證；目前系統效能數據僅能支持「查表推論遠低於完整孤立森林推論成本」此一方向性結論。
...
\subsubsection{結論}
本專題的核心結論是：DDoS 偵測模型若要進入 Linux kernel fast path，不能只追求離線準確率，而必須同時滿足 verifier-friendly、可由 datapath 重建、整數化、低推論成本與低誤報操作點等部署條件。基於此訴求，本專題提出並實作一條 Python 離線模型、Rust 使用者空間控制面與 eBPF XDP／TC 資料路徑三者對齊的低延遲 DDoS 偵測與緩解管線。回應導論所提之四項研究問題：（一）在 eBPF 相容性方面，本系統以 Contract5 feature contract 與 $N=2$ 分位桶將五個特徵轉換為 5-bit 索引，並以 32-entry bucket student 評分表取代樹狀巡訪，使核心快速路徑僅需整數比較與查表；（二）在特徵整數化方面，比例特徵改以交叉乘法表示，避免浮點與高成本除法；（三）在資料路徑分工方面，XDP 負責早期解析、黑名單與 SYN-cookie，TC 負責特徵重建與評分，達成效能隔離；（四）在攻擊污染隔離方面，雙草圖閘控更新可在攻擊比例 $\rho \geq 30\%$ 時觸發 AttackFreeze，將 FNR 維持於基準值 0.182，而樸素更新則惡化至 1.000。

實驗以 CIC-IDS-2019 為範圍，deployed student 在 FPR $\leq 1\%$ 操作點達 F1 = 0.901、Recall = 0.821，與監督式 Random Forest／Logistic Regression 相近，並對多種 UDP 反射／放大型 DDoS 達 99\% 以上 Recall。進一步的對照實驗顯示，直接回歸 teacher score 雖能保留 teacher ranking，卻會繼承 teacher 在低 FPR 操作點下的不可用性；bucket student 反而透過離散化重塑分數分布，取得更可部署的操作點表現。分位桶數量敏感度亦顯示 $N=2$ 並非單純的 table size 妥協，而是操作點穩健性與 eBPF 可行性的交集；串流邊界實驗則說明，未設閘門的 Periodic 更新會在 attack contamination 下崩潰，而 Gated 更新可維持接近 Static 的乾淨基線。

整體而言，本專題的貢獻不在追求最高離線 AUC，而在於建立一條可驗證、可部署且操作點穩健的核心快速路徑偵測與緩解管線，同時誠實界定其盲區與適用範圍。目前效能證據可支持「查表式推論成本遠低於完整孤立森林推論」此一方向性結論；完整的核心端吞吐量、CPU 負擔與端到端緩解延遲仍需後續實機量測補齊。

\subsubsection{未來工作}
本研究尚有以下延伸方向：
\begin{itemize}
  \item \textbf{實機效能驗證：} 於固定 Linux kernel、網卡與流量產生環境下，量測 no mitigation、static blocklist、userspace IF 與 eBPF bucket policy 的 pps、CPU overhead、kernel map lookup latency 與端到端 mitigation latency，將目前 userspace proxy 的方向性效能結論補齊為完整基準測試。
  \item \textbf{正式 teacher-student distillation：} 將目前以二值特徵模型產生 32-entry table 的工程流程，補強為 Contract5 continuous teacher 到 bucket student 的正式 knowledge distillation，例如以 CE／KL loss 或 score distribution matching 訓練 bucket policy，並比較其與現況二值化流程在 FPR 操作點下的差異。
  \item \textbf{Runtime Layer 2 推論：} 將完整 Isolation Forest 或其他較高成本模型接入 userspace，只針對 fast path 標記為 \code{MONITOR}、gray-zone 或取樣上報的 flow 進行二階段判斷，避免讓完整模型進入 per-packet fast path。
  \item \textbf{盲區與多層防線擴充：} 針對 UDP-lag、WebDDoS／LOIC-HTTP 等目前 Contract5 無法表達的攻擊型態，研究可由 datapath 重建的新特徵或 userspace／application-layer 語意；對 SYN flood 則進一步驗證 SYN-cookie path 與 bucket scorer 之間的分工。
