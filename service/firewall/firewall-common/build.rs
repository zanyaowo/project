...
fn main() {
    ...
    let (
        ...
        // quantile_bound_size 預設 10 = 2 banks × 5 features（double buffering）
        (
            1024u32, 65536u32, 4096u32, 10u32, 32u32, 1u32, 65536u32, 1u32,
        )
    };
    ...
    let code = format!(
        "pub const BLOCK_LIST_SIZE: u32 = {};\n\
           ...
           pub const BOUNDARY_META_SIZE: u32 = {};\n",
        block_list_size,
        session_table_size,
        event_ring_buffer_size,
        quantile_bound_size,
...
fn parse_map_sizes(contents: &str) -> (u32, u32, u32, u32, u32, u32, u32, u32) {
    ...
    (
        ...
        maps.get("model_config_size")
            .and_then(|v| v.as_integer())
            .unwrap_or(1) as u32,
        maps.get("stats_ring_buf_size")
            .and_then(|v| v.as_integer())
            .unwrap_or(65536) as u32,
        maps.get("boundary_meta_size")
            .and_then(|v| v.as_integer())
            .unwrap_or(1) as u32,
    )
}
